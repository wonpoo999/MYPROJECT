...
export default function Register() {
  ...
  const clientError = useMemo(() => {
    ...
    if (!["MALE","FEMALE"].includes(String(form.gender).toUpperCase()))
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (login?.token){
        localStorage.setItem("dc_token", login.token);
        localStorage.setItem("dc_uid", String(login.uid ?? ""));
        nav("/", { replace:true });
        return;
      }

      // (폴백) 자동 로그인 실패시 로그인 페이지로
      ...
      <div className="auth-wrap">
        <div className="auth-card">
          <header className="card-head">
            <NavLink to="/" aria-label="메인으로"><Logo size={60}/></NavLink>
            <div>
